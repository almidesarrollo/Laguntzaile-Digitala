...
import com.asistencia.digital.monitor.activities.MainGeo;
...
public class ControlSensores implements  SensorEventListener{
    ...
    public static final int MAX_ACEL=40,MIN_ACEL=20,MAX_CAD=75;
   ...
   // private float[] datosMagneticos = new float[3];           // Magnetometer
    ...
    private float datosextranos;
    private String peneiruken;
    private long currentmoment;
    private boolean hasFall;

    private  float normsAcec;
    //private  float normH;
...
*/
    ...
    public ControlSensores(Context c)
    {
        ...
        alertMod=0;
         cMain=c;
        mngrSensores = (SensorManager)c.getSystemService(Context.SENSOR_SERVICE);

        hSensor=mngrSensores.getDefaultSensor(Sensor.TYPE_HEART_RATE);

        // gsensor =  mngrSensores.getDefaultSensor(Sensor.TYPE_GRAVITY);
    ...
    {
        //mngrSensores.registerListener(this,gsensor,SensorManager.SENSOR_DELAY_NORMAL);
       // for (Sensor sensor:hSensor) {
          mngrSensores.registerListener(this,hSensor,SensorManager.SENSOR_DELAY_NORMAL);
     //   }
     /*   for (Sensor sensor:msensor) {
            mngrSensores.registerListener(this,sensor,SensorManager.SENSOR_DELAY_NORMAL);
        }*/
...
*/
    ...
    public void onSensorChanged(SensorEvent event) {
        MainGeo.txt5.setText("Normal de aceleracion"+normsAcec+"\n Acel:x"+datosAccec[0] +"\n Acel:y"+datosAccec[1] +"\n Acel:z"+datosAccec[2]
                +"\n MINIMOACELERACION="+MIN_ACEL +" LIMITEACELERACION="+MAX_ACEL +"LIMITECAIDA"+MAX_CAD+"\n MAXACELELRACION="+maxMov+"\n MAXCAIDA="+maxCad+"\n AceleracionQuePilla="+alertMod+"\n CaidaQuepilla="+alertCad+
                "DatosExtraños="+peneiruken+"-"+datosextranos
                +"\n Datitos de mi corazon:"+opcional);
        switch (event.sensor.getType()) {
           case Sensor.TYPE_GRAVITY:
            ...
            case 0:
                break;
            default:
                peneiruken=event.sensor.getName();
                datosextranos=event.values[0];
       ...
       // final float normsqA = (datosMagneticos[0] * datosMagneticos[0] + datosMagneticos[1] * datosMagneticos[1] + datosMagneticos[2] * datosMagneticos[2]);
        ...
        if(event.sensor.getType()==Sensor.TYPE_ACCELEROMETER || event.sensor.getType()== Sensor.TYPE_MAGNETIC_FIELD || event.sensor.getType()== Sensor.TYPE_GRAVITY ) {
      ...
      /*  if (normsqA < freeFallGravitySquared) {
       ...
       /* if (normH < maxCad) {
        ...
        if (hasFall) {
            if (System.currentTimeMillis()-currentmoment>10000) {
                hasFall = false;
            }
            if(System.currentTimeMillis()-currentmoment>5000) {
                if(normsAcec < 9.8)
                    createIssue(R.integer.const_issue_caida);
                    hasFall = false;
                }
            }

        } else if (event.sensor.getType() == 87) {
            MainGeo.txt5.setText("Pulso:"+opcional);
            if (opcional > 83) {
                createIssue(R.integer.const_issue_pulso);
            }
            if (opcional < 55) {
                createIssue(R.integer.const_issue_pulso_bajo);
            }
        }
    }
    ...
    private void createIssue(int type) {
        Toast.makeText(cMain, "TE CAISTE", Toast.LENGTH_LONG).show();
        AsistRestApiClient restApi = new AsistRestApiClient(cMain);
        restApi.createIssue(type);
    }
